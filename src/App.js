...
class App extends React.Component {
  ...
  componentWillMount (){
    ...
    if(auth && Date.parse(auth.auth.valid_until) > Date.now()){
      ...
      ReactDOM.render(
          ...
          <button type="button" className="btn btn-primary custom-btn p-3  " id="launch-modal" data-toggle="modal" data-target="#exampleModalCenter"><h4 className="my-0">View your profile</h4></button>,
          </div>,
  ...
  loginPostFetch(data){
    ...
    ReactDOM.render(
      <div className="text-center">
      <h4> You are already logged in.  </h4>
      <button type="button" className="btn btn-primary custom-btn p-3  " id="launch-modal" data-toggle="modal" data-target="#exampleModalCenter" data-backdrop="static"><h4 className="my-0">View your profile</h4></button>,
      </div>,
      document.getElementById('register-root')
    );
  }
