...
const TeamPosts = (props) => {
    ...
    const submission_action = (title, text) => {
        cancellablePromise(profile.postForumPost(title, text), async (post_uuid) => {
    ...
    React.useEffect(() => {
        cancellablePromise(profile.getMyForumPosts(), async (posts) => dispatchMyPosts({type : "success", payload : {posts : posts}}), async (err) => dispatchMyPosts({type : "error", payload : {}}));
        cancellablePromise(profile.getAllForumPosts(), async (posts) => dispatchAllPosts({type : "success", payload : {posts : posts}}), async (err) => dispatchAllPosts({type : "error", payload : {}}));
    }, []);
        ...
        <div>
            <div>
                ...
                {!myPostsState.is_loading && !myPostsState.display_err ?
                    <ForumPostListView posts_list={myPostsState.posts} post_click_action={openForumPost}/>
                    :
            ...
            <div>
                ...
                {!allPostsState.is_loading && !allPostsState.display_err ?
                    <ForumPostListView posts_list={allPostsState.posts} post_click_action={openForumPost}/>
                    :
            ...
            <div>
                ...
                <PostPreviewView submission_action={submission_action} submission_validator={submission_validator} display_err={errState.display_err} err_msg={errState.err_msg}/>
            </div>
        </div>
    );
};
