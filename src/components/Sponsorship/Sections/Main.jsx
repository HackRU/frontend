    ...
    render() {
        ...
        for (let i = 0; i < keys.length; i++) {
            if (sponsorshipLinks[keys[i]].enabled && !sponsorshipLinks[keys[i]].hideLink) {
                navSponsorship.push(
                    <p className="lead"
                        key={keys[i]}><a href={sponsorshipLinks[keys[i]].url}
        ...
        if (!this.props.isMobile) {
            return (
                <Container fluid
                    ...
                    <Row style={{ minWidth: "100%", minHeight: "100vh" }}
                        ...
                        <Col xs={8}
                            style={{ display: "block" }}>
                            <h2 style={{ marginTop: 20, color: theme.primary[0] }}
                                className="display-1">Sponsor HackRU</h2>
                        </Col>
                        <Col xs={2}
                        ...
                        <div style={{ position: "absolute", bottom: 10, textAlign: "center", width: "100%", paddingBottom: 20 }}>
                            <Button href="#about"
                                outline
                                style={{ border: "none"}} ><Icon name="chevron-down" /> Explore</Button>
                        </div>
                    </Row>
                </Container>
            );
            ...
            return (
                <Container fluid
                    id="sponsorship-section"
                    style={{ zIndex: 3, width: "100%", minHeight: "100vh", textAlign: "center", userSelect: "none" }}>
                    <Row style={{ minWidth: "100%", minHeight: "100h" }}
                        className="d-flex align-items-center">
                        <Col style={{ display: "block" }}>
                            ...
                            <h2 style={{ marginTop: -10, color: "#FFF5e8" }}
                                className="display-5 theme-font">Sponsor HackRU Fall 2023</h2>
